...
for (s in 2:gibbsIts) {
    ...
    for (w in 3:num_freqs) {
        for (k in 1:K) {
            ...
            for (j in 1:d) {
                ...
                a <- 1
                b <- 2 - mu
                c <- LL*tau2_s[s-1] + 1 - 2*mu
                dd <- LL*tau2_s[s-1] - ajk*tau2_s[s-1] - mu
    ...
    tau2_s[s] <- 1/rgamma(1, tau2_a + .5*(num_freqs-1)*K*d,
...
plot(upper_q[1, k, ], type = "l", lty = 2, ylim = c(0, 5))
...
plot(tau2_s[1:gibbsIts], type = "l")

summary(tau2_s[gibbsPostBurn])
