...
# U_kl0, signal eigenvectors
U_kl0 <- array(NA, c(P, d, K, Tt))
for (t in 1:(Tt/2 - 1)) {
    for (k in 1:K) {
        U_kl0[, , k, t] <- rCMACG(P, d, Sigmal0[, , t])
        U_kl0[, , k, Tt - t] <- Conj(U_kl0[, , k, t])
    }
}
for (k in 1:K) {
    U_kl0[, , k, Tt/2] <- rMACG(P, d, diag(P))
    U_kl0[, , k, Tt] <- rMACG(P, d, diag(P))
}

# Lambdakl0, factor power spectra
...
plot(Lambdakl0[1, 1, ], type = "l")
lines(Lambdakl0[2, 1, ], col = 2)

plot(Lambdakl0[1, 2, ], type = "l")
lines(Lambdakl0[2, 2, ], col = 2)
...
for (k in 1:K) {
    # handles through Tt/2-1 and Tt/2, since the sigmakl02 are in that order.
    for (t in 1:(Tt/2)) {
        Sl[, , k, t] <- sigmakl02[k, t] * 
            (U_kl0[, , k, t] %*% diag(Lambdakl0[, k, t]) %*% t(Conj(U_kl0[, , k, t])) +
                  diag(P))
        diag(Sl[, , k, t]) <- Re(diag(Sl[, , k, t]))
    }
    
    # at Tt/2 (see the first loop)
    
    # TODO modify this loop to use the correct sigmakl02 and other parameters.
    # from Tt/2 + 1 to Tt-1
    for ( t in (Tt/2 + 1):(Tt-1) ) {
        Sl[, , k, t] <- sigmakl02[k, Tt - t] * 
            (U_kl0[, , k, t] %*% diag(Lambdakl0[, k, t]) %*% t(Conj(U_kl0[, , k, t])) +
                  diag(P))
        diag(Sl[, , k, t]) <- Re(diag(Sl[, , k, t]))
    }
    
    # TODO modify this step to use the correct sigmakl02 and other parameters.
    # at Tt
    
    Sl[, , k, Tt] <- sigmakl02[k, num_freqs + 2] * 
            (U_kl0[, , k, Tt] %*% diag(Lambdakl0[, k, Tt]) %*% t(Conj(U_kl0[, , k, Tt])) +
                  diag(P))
        diag(Sl[, , k, Tt]) <- Re(diag(Sl[, , k, Tt]))
}
